...
class GameScene: SKScene {
    ...
    func createEnemy(from placeholder: SKSpriteNode, name : String) -> Enemies? {
        guard let enemyType = EnemyType(rawValue: name) else { return nil }
        let enemy = Enemies(type: enemyType)
...
extension GameScene: SKPhysicsContactDelegate {
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        case PhysicsCategory.bird | PhysicsCategory.enemy:
            if let enemy = contact.bodyA.node as? Enemies {
                if enemy.impact(with: Int(contact.collisionImpulse)) {
                    enemies -= 1
                }
            } else if let enemy = contact.bodyB.node as? Enemies {
                if enemy.impact(with: Int(contact.collisionImpulse)) {
                    enemies -= 1
                }
            }
